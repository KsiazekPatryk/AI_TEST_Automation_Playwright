# DELETE /books/{id} Negative Scenarios & Business Validations
...
- Missing path parameter: not directly executable against `/books/{id}` because the documented endpoint requires the path segment.
...
Document that the OpenAPI contract requires `{id}` in the path and no `DELETE /books` operation exists.
...
Missing `id` path segment; request path would be `/books`, which has no documented DELETE operation.
...
Not documented in OpenAPI because `DELETE /books` is not a documented operation.
...
- Assert OpenAPI contains `DELETE /books/{id}`.
- Assert OpenAPI does not contain `DELETE /books`.
- Do not assert a runtime status code for `DELETE /books` because the operation is outside the documented contract.

# Notes

- OpenAPI documents no `400`, `401`, `403`, `404`, `409`, or validation-error responses for `DELETE /books/{id}`.
...
- `DELETE /books/{id}/cover` is documented separately for cover removal and is intentionally excluded from negative scenarios for book deletion.
